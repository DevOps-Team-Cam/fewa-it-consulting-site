...
try {
    ...
    $emailContent = file_get_contents('rd-mailform.tpl');
    ...
    preg_match("/(<!-- #\{BeginInfo\} -->)(.|\s)*?(<!-- #\{EndInfo\} -->)/", $emailContent, $tmp, PREG_OFFSET_CAPTURE);
    foreach ($_POST as $key => $value) {
        if ($key != "counter" && $key != "email" && $key != "message" && $key != "form-type" && $key != "g-recaptcha-response" && !empty($value)){
            ...
            $emailContent = str_replace("<!-- #{EndInfo} -->", $info, $emailContent);
        }
    }
