...
$search_emailContent = preg_replace('/\+/', ' ', $_GET['emailContent']);
preg_match_all("/\#\{((?!title|href|token|count)[a-z]*)\}/", $search_emailContent, $emailContent_tokens);
$emailContent_tokens = $emailContent_tokens[1];
...
foreach ($files as $file) {
  ...
  for ($j = 0; $j < count($emailContent_tokens); $j++) {
    if (preg_match("/\<meta\s+name=[\'|\"]" . $emailContent_tokens[$j] . "[\'|\"]\s+content=[\'|\"](.*)[\'|\"]\>/", $contents, $res)) {
      $final_result[$file_count][$emailContent_tokens[$j]] = $res[1];
    }
  }
    ...
    <ol class="search-list">
      ...
      for ($i = 0; $i < count($final_result); $i++) {
        ...
        <li class="search-list-item">
            ...
            <?php
              ...
              $emailContent = preg_replace(["/#{title}/","/#{href}/","/#{token}/","/#{count}/"],$replacement, $search_emailContent);
              for ($k = 0; $k < count($emailContent_tokens); $k++){
                if (isset($final_result[$i][$emailContent_tokens[$k]])){
                  $emailContent = preg_replace("/#{" . $emailContent_tokens[$k] . "}/", $final_result[$i][$emailContent_tokens[$k]], $emailContent);
                }else{
                  $emailContent = preg_replace("/#{" . $emailContent_tokens[$k] . "}/", " ", $emailContent);
                }
              }
      ...
      if (isset($_GET['liveSearch']) and $_GET['liveSearch'] != "" and $match_count != 0) {
        ...
        <li class="search-list-item-all">
            <a href='search-results.html?s=<?php echo $_GET['s']; ?>&amp;filter=<?php echo $search_filter_init; ?>'
                class="search-submit">
                <?php
            echo "See other ";
            echo $sum_of_results;
